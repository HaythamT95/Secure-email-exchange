...
void hexStringToBinary(char* hexString, BYTE* binaryData, int binaryLen) {
	...
	for (int pos = 0; ((pos < (binaryLen * 2)) && (pos < strlen(hexString))); pos += 2)
	{
		idx0 = (uint8_t)hexString[pos + 0];
		idx1 = (uint8_t)hexString[pos + 1];
		binaryData[pos / 2] = (uint8_t)(hashmap[idx0] << 4) | hashmap[idx1];
	};
}
...
int blockEncrypt(cipherInstance* cipher, keyInstance* keyInst, BYTE* input,
	...
	//outputBinary("aa", input, BLOCK_SIZE);
